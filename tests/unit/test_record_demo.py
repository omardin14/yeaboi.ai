...
class TestDemoScript:
    ...
    def test_marker_sets_are_disjoint(self):
        ...
        If a fragment appeared on both, an await would match the screen it was
        leaving and the recording would race ahead of the transition.
        """
    ...
    def test_starts_by_awaiting_the_menu(self):
        # The first screen after the splash, now that there is neither a split
        # nor a door.
        assert record_demo.DEMO_SCRIPT[0][0] == "await"
        assert record_demo.DEMO_SCRIPT[0][1] == record_demo.MODE_SCREEN_MARKERS
