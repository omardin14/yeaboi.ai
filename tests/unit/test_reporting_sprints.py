...
from scrum_agent.reporting import sprints
...
class TestListSprints:
    ...
    def test_falls_back_to_azdo(self, monkeypatch):
        monkeypatch.setattr("scrum_agent.tools.jira.jira_list_sprints", lambda *a, **k: [], raising=False)
        monkeypatch.setattr(
            "scrum_agent.tools.azure_devops.azdevops_list_sprints",
    ...
    def test_falls_back_to_plan(self, monkeypatch):
        monkeypatch.setattr("scrum_agent.tools.jira.jira_list_sprints", lambda *a, **k: [], raising=False)
        monkeypatch.setattr("scrum_agent.tools.azure_devops.azdevops_list_sprints", lambda *a, **k: [], raising=False)
        state = {
    ...
    def test_limit_keeps_most_recent(self, monkeypatch):
        ...
        monkeypatch.setattr("scrum_agent.tools.jira.jira_list_sprints", lambda *a, **k: rows, raising=False)
