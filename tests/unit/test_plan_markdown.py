...
class TestBuildPlanMarkdown:
    ...
    def test_sprint_overview_table(self):
        from tests._node_helpers import make_sample_sprints, make_sample_stories

        state = {"sprints": make_sample_sprints(), "stories": make_sample_stories(), "velocity_per_sprint": 10}
        md = build_plan_markdown(state)
        assert "| Sprint | Goal | Capacity | Used |" in md
        first = state["sprints"][0]
        assert f"| **{first.name}** |" in md
