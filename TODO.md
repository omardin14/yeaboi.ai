...
### Phase 32.2: Analysis preview — feedback-driven Regenerate
...
- [x] `tools/team_learning.py` — `compute_headline_stats()` + `compute_recommendations()` (pure, shared by overview/cards/digest/exports), `ANALYSIS_GLOSSARY` (Churn/Δ/Spill%/Cycle/variance/confidence definitions), `_generate_analysis_narrative()` (one LLM call → executive summary + per-section "what this means"; digest-fed prompt, parse → deterministic fallback, never raises) hooked at the end of `_run_parallel_analysis` into `examples["narrative"]` (persists + resumes for free)
- [x] `ui/mode_select/screens/_analysis_sections.py` (new) — mechanical extraction of the ~1,775-line `_build_team_analysis_screen` into `_TaCtx` + 24 per-section builders + `_TA_CARDS` registry (7 cards: Velocity & Sprints / Team Members / Estimation & Points / Workflow & DoD / Writing Style / Trends & Repos / Recommendations), `_ta_overview` (headline stats + AI summary + selectable card list), `_ta_narrative_block`, `_ta_glossary_lines`
- [x] `_screens_secondary.py` — `_build_team_analysis_screen` re-built on `view=`/`selected_card=`/`actions=` (replaces `page=`); breadcrumb subtitle; selection auto-scrolls into view; fixed pre-existing scrollbar off-by-one that clipped the buttons' bottom border
- [x] `ui/mode_select/__init__.py` — one shared `_run_team_analysis_results()` event loop replacing the three duplicated page loops (fresh analysis, saved-profile View, resumed); callers keep their own Continue handling
- [x] `team_profile_exporter.py` — HTML + MD gain an Executive Summary section (summary + per-area explanations, escaped) and the churn/Δ/spill glossary under the sprint table
- [x] Tests — `TestAnalysisOverview`/`TestAnalysisSectionDetail` (all 7 cards, glossary, narrative shown/omitted, empty profiles, terminals), `TestComputeHeadlineStats`/`TestComputeRecommendations`/`TestGenerateAnalysisNarrative` (digest prompt, fences, fallbacks, back-fill), narrative store round-trip + exporter escaping; `make test` (3626 passed) + `make lint` clean

### Phase 33: Unified logging — one format, per-mode logs, live log-level setting
- [x] `logging_setup.py` (new) — single owner of format/level/rotation/handler lifecycle: `configure_logging()` (always-on `logs/tui/yeaboi.log`), `attach_mode_handler`/`detach`/`mode_log()` context manager (`logs/<mode>/<mode>.log`), `attach_session_log`/`detach_session_log` (rotating planning logs), `apply_level()` (live retune of logger + all handlers); all rotating 2 MB × 3, shared format, unified WARNING fallback
- [x] `cli.py` — both inline handler blocks replaced with `configure_logging()` (TUI + `--non-interactive`); headless standup (`--standup-run`) migrated `standup-run.log` → rotating `standup/standup.log`
- [x] `persistence.py` — `attach_session_logger`/`remove_session_logger` delegate to `logging_setup`; `delete_project` now globs `{id}.log*` so rotation backups are removed too
- [x] `ui/mode_select/__init__.py` — standup/retro/performance/reporting dispatches wrapped in `with mode_log(...)`; analysis branch uses explicit attach/detach at both exits
- [x] `config.py` — `VALID_LOG_LEVELS` + `set_log_level()` (dotenv `set_key` to `~/.yeaboi/.env` + `os.environ`, ValueError on invalid)
- [x] Settings TUI — new **Log Level** cycle button (`Configure / Log Level / Back`): cycles DEBUG→INFO→WARNING→ERROR via `_next_log_level()`, persists with `set_log_level()`, applies live with `apply_level()`, row updates instantly; `"Log Level"` registered in `_BTN_COLORS`
- [x] Exhaustive instrumentation sweep — INFO on every user action (page open/close, button activations, generate/export/configure, menu selections, editor open/save/cancel, provider/verification steps, splash), WARNING/ERROR on every failure path, DEBUG in previously-silent excepts (tools/github, jira, notion, team_learning, retro server); retro board mutations logged from HTTP threads (thread-safe stdlib logging); zero logging in per-frame builders/render paths; no secrets/tokens/join-codes/user content logged
- [x] Tests — `test_logging_setup.py` (17: attach idempotency, rotation config, detach-on-raise, session replace, apply_level, level from env); `set_log_level` round-trip + validation in `test_config.py`; Log Level button render + `_BTN_COLORS` + `_next_log_level` cycle in `test_shared_components.py`; `delete_project` removes rotation backups in `test_persistence.py`; `make lint` clean, unit + integration suites green

## Phase 34: Splash/intro wordmark alignment fix
- [x] `ui/splash.py` — mode-intro wordmarks (e.g. STANDUP) rendered with per-line `justify="center"`, which rstrips each row and re-centres it by its own width, shifting rows with shorter trailing content (the P glyph) and breaking the letters; replaced with whole-block centring (`_block_left_pad` + left-anchored rows) in `_build_splash_frame`/`_build_shine_frame`; regression tests in `tests/test_splash.py`; `make test` + `make lint` clean

## Phase 35: Update check + in-app AI changelog
- [x] `update_check.py` (new) — one-shot daemon-thread PyPI check (`https://pypi.org/pypi/yeaboi/json`, stdlib urllib, 3 s timeout, swallow-all-errors telemetry pattern): `parse_version`/`is_newer` (dependency-free tuple compare, unparseable never flags), `detect_upgrade_command` (uv tool vs pipx from `sys.executable`), idempotent `start_background_check` (skips `0.0.0+dev`), `get_update_status` UI/test seam; kicked off at the top of `select_mode`
- [x] Mode-select bottom-left version row — `_build_version_row` in `_screens.py` (lazy-reads update state like `_build_tip_rows` reads tips config; no builder-signature change): `vX.Y.Z · c changelog` normally, `vX.Y.Z → vNEW · <upgrade cmd> · c changelog` in the warm tip accent when outdated (command dropped under 72 cols); reserved-height math extended by one row; sits opposite the music bar (bottom border, right)
- [x] `changelog.py` + `changelog_data.json` (new, ships in the wheel automatically) — frozen `ChangelogEntry`/`ChangelogHighlight`, fixed area vocabulary (analysis/planning/standup/retro/performance/reporting/usage/settings/general), `AREA_COLORS` mapped to each mode's accent, graceful `load_changelog()` (missing/corrupt → `[]`, unknown areas → general); backfilled user-friendly entries for all 27 released versions v1.0.0…v2.12.0 from git history
- [x] Changelog page — `CHANGELOG_THEME` + `changelog_title()` + baked CHANGELOG ANSI-Shadow wordmark; `_build_changelog_screen` in `_screens_secondary.py` (upgrade banner when outdated, per-version heading + summary + highlight bullets with colour-coded area tags, standard viewport/scrollbar/Back button); `_run_changelog_page` + inline `c` hotkey in the mode-select Phase-1 loop (wordmark intro, no Phase-2 transition)
- [x] `auto-version.yml` — Claude now writes/updates the new version's `changelog_data.json` entry alongside every bump (prepend newest-first, fixed area vocabulary, JSON validity check); guard 1 gains changelog-only mode for manual bumps; guard 3 excludes entry-only edits from triggering a release
- [x] Tests — `test_update_check.py` (28: version parsing, installer detection, mocked PyPI fetch, thread idempotency), `test_changelog.py` (bundled-data integrity + graceful loading), `test_changelog_screen.py` (render/height/banner/tags/scroll), `test_version_row.py` (row states, narrow-width truncation, mode-screen height invariant); `make lint` clean, unit + integration suites green (contract failures pre-existing/environmental)

## Phase 36: Ctrl+V screenshot paste in TUI textboxes (multimodal LLM input)
- [x] `ui/shared/_input.py` — `\x16` → `"ctrl+v"` in the `read_key` chokepoint (same app-wide pattern as the Ctrl+P/O music chord); pty decode test
- [x] `clipboard.py` (new, stdlib-only) — `read_clipboard_image()` → `(bytes, mime)`: macOS `pngpaste` fast-path → `osascript «class PNGf»` → `«class JPEG»` (UTF-8 guillemet hex parse); Linux `wl-paste` → `xclip`; 10s timeout, graceful `None`; `tests/unit/test_clipboard.py`
- [x] `paths.py` — `ATTACHMENTS_DIR` + `get_attachments_dir(scope_id)` (`~/.yeaboi/attachments/<scope>/`)
- [x] `ui/shared/_attachments.py` (new) — `handle_ctrl_v()` (read → 4.5 MB guard → save `img-<8hex>.png|.jpg` → `[image #N]` chip), `referenced_images()` (surviving-chip scan: deleting a chip detaches its image), `unsupported_notice()`; `tests/unit/test_attachments.py`
- [x] `agent/llm.py` — `load_image_b64` / `build_multimodal_content` (portable LangChain image blocks, all 4 providers) / `invoke_with_images` (drop-in for `invoke([HumanMessage(prompt)])`, one text-only retry when a non-vision model rejects blocks); tests in `test_llm.py`
- [x] `agent/state.py` — `pasted_images` / `review_feedback_images` / `chat_images` (`list[str]` file paths; auto-serialize, survive `--resume`; `messages` stays text-only); round-trip tests in `test_state.py`
- [x] Planning wiring — description input (`_phase_description_input` + `_attachment_scope` stash) and questionnaire answers (`_question_input_loop`) → `pasted_images` → analyzer via `invoke_with_images`; review-edit feedback (`_get_edit_input` + both `_phases.py` callers) → `review_feedback_images` → feature/story/task(review)/sprint nodes, cleared on Accept/export-only (+ legacy REPL accept); chat (`_phase_chat` → `chat_images` → `_attach_chat_images` in `call_model`/`call_model_with_tools`, invoke-time only, cleared after consumption); analyzer node tests
- [x] Standup — `_standup_read_line` attachments kwarg; My Update + Generate flow save images (`StandupStore.save_my_update(images=)`, idempotent `images_json` ALTER + fresh-schema column, `get_my_update_images` prunes missing files) → `run_standup`/`_summarize_members` → summary LLM call; store + engine tests
- [x] Performance — `_performance_get_transcript` returns `(transcript, images)`; `complete_one_on_one(images=)` → `_invoke_llm` → `invoke_with_images`; engine tests
- [x] Artifact editors (`edit_buffer_loop`, `edit_story`) deliberately reject image paste with a subtitle notice — a chip would become literal artifact text; screenshots belong in Edit/Regenerate feedback
- [x] Non-LLM fields (provider API key/model/VC token, Confluence/Notion/issue-tracking credentials, questionnaire import path) — Ctrl+V shows "Images are not supported in this field"
- [x] README — Screenshot Paste feature blurb + "Multimodal Content Blocks" learning section under Prompt Construction; `make test` + `make lint` clean
- [x] Discoverability — `_image_hint()` in `_screens_input.py` (tips-gated, "📷 Ctrl+V paste screenshot", "(not ⌘V)" on macOS) appended to the hint line of every image-enabled field: description, free-text questions (accordion + classic), edit/regenerate feedback, chat, standup My Update + performance transcript (`show_image_hint` flag on `_build_standup_input_screen`); tests in `test_tips_ui.py`

## Phase 37: Standup TUI redesign — multi-line update, alias attribution, self-report-as-context, sectioned page
- [x] Multi-line "Your update" box — `_standup_read_line` inserts `\n` on Alt+Enter/Ctrl+N when `box_rows > 1`; `_build_standup_input_screen` chunks newline-aware (split on `\n`, then width-wrap); `box_rows=6` at both call sites (My Update step 2/2 + pre-Generate prompt); "Alt+Enter for a new line" hint; render tests
- [x] Alias-aware activity attribution — `engine._normalize_author` (case-insensitive + email local part), `_build_alias_map` (member names + standup user's configured `my_aliases` + auto-detected local git `user.name`/`user.email` via `_detect_git_identity`), alias-driven `_group_activity_by_author`; covers every collector source (GitHub/Jira/AzDO/Confluence/Notion/local git) since matching happens on the normalized item stream
- [x] Roster fix — plan roster no longer swallows activity: the standup user is always a member, and activity authors matching no alias become members themselves; "No activity sources configured" warning when the collector has nothing enabled
- [x] `my_aliases` persistence — `standup_config` column (idempotent ALTER + fresh schema), `save_config`/`load_config` round-trip, new Configure step (6/7); store migration tests
- [x] Self-report as supporting context — `MemberUpdate.self_report` field (defaulted, backward-compat) + `source="combined"`; `_summarize_members` sends every member (activity + self_report) to one LLM call; prompt rewritten (grounded-in-activity requirement, own words shown separately); fallback keeps activity titles + self_report; render/export show "✍ In their words" + analysis in plaintext/Rich/Markdown/HTML; engine + store + export tests
- [x] Sectioned standup page — new `_standup_sections.py` registry (`_StandupCtx`, computed card order: Team Summary / Sprint & Confidence / one card per member / Activity / Schedule / Notices-when-warned, titles + teasers + detail builders, STANDUP_THEME only); `_build_standup_screen` rewritten with `view`/`selected_card`/`actions` (overview headline + ▸ section list with selection auto-scroll; detail views with `Overview › …` breadcrumb); `_run_standup_page` adopts the analysis loop (Up/Down selects on overview, free-scrolls detail; Open expands, Back/Esc collapses; Export moved to detail views); member detail preserves Alt+Enter paragraph breaks; screen tests for all views
- [x] `make lint` clean; unit + integration suites green (contract failures pre-existing/environmental)

## Phase 38: Standup follow-up — button row fix + working-day activity window
- [x] Clipped/overlapping buttons root-caused and fixed at the source — `build_scrollbar` no longer returns a trailing newline (rendered as an extra row, pushing the buttons' `╰──╯` border off the fixed-height panel); redundant `.rstrip()` workaround removed from the team-analysis builder; fixes the same latent clip on performance/reporting/retro/changelog; render tests assert the bottom border stays on-screen
- [x] Overview reduced to 4 buttons with a two-zone focus model — "Open" removed; ↑/↓ focuses the section list (no button highlighted, `action_sel=-1`), Enter opens the selected section directly; ←/→ moves focus to [Generate · My Update · Configure · Back] where Enter presses the highlighted button; hint line "↑/↓ section · Enter to open · ←/→ buttons"
- [x] Working-day activity window — `collector.previous_working_day_start(today)` (Mon–Fri, local midnight): Sat/Sun/Mon runs reach back to Friday 00:00, midweek runs cover the FULL previous day + today up to the run moment; `collect_recent_activity(since=)` threads a datetime to all six source helpers (`jira`/`confluence` absolute date literals, `azure_devops` midnight-exact `@Today - N`, `github`/`notion` UTC datetime cutoffs, `local_git` ISO `--since`); `run_standup(days=None)` default uses the window (explicit `days` keeps the legacy now−N behavior for tests)
- [x] Window surfaced — `StandupReport.activity_window` (defaulted, backward-compat, `.get()` in store), shown in the Activity detail view, plaintext delivery, and Markdown export
- [x] Tests — previous_working_day_start (Sat/Sun/Mon/midweek/tz-aware), collector `since` passthrough, per-helper `since` honoring (JQL/CQL/WIQL/GitHub/Notion/local git), engine Monday→Friday window + `activity_window` on the report + legacy `days` path, store round-trip, scrollbar/button-border/focus render tests; `make lint` clean, unit + integration green
- [x] Zero-config identity resolution — `engine._detect_tracker_identity()` (Jira `myself()` displayName + email; GitHub token login, token-gated) + `_detect_git_identity` global-config fallback; when `STANDUP_USER_NAME` is the default "Me" the detected Jira displayName replaces the placeholder (self-reports re-keyed), detected identities fold into the alias map, and roster/plan entries that alias-match the user are merged into their single card — no more separate "Me (you)" and "Omar Din" cards; `StandupReport.my_name` (defaulted, store `.get()`) drives the "(you)" tag in the TUI
- [x] Team roster from the tracker — when the plan has no `selected_team_members`, the engine reuses `performance/roster.fetch_roster()` (Jira/AzDO assignees, ~30-day window) so the whole team appears with per-member cards even when they have no activity in today's window ("No activity detected."); the user's card always sorts first; engine tests for rename/merge/roster + tracker-identity unit tests (detection stubbed in `_patch_common` for determinism)
- [x] Live Generate progress — pressing Enter on the update box no longer freezes: `run_standup(on_progress=)` reports each phase (collect → sprint → identities → AI summaries → save/export; best-effort, a broken callback never breaks the run), the TUI runs the pipeline on a worker thread and renders `_build_standup_progress_screen` (STANDUP_THEME spinner + elapsed + ✓/▸ phase steps, analysis-progress pattern) at ~30 fps until done; engine phase-order + broken-callback tests, render tests

## Phase 39: Standup — accurate per-member activity collection & attribution
- [x] Jira actor-level fetch — `jira_recent_activity` expands `changelog` + `comment` on the one search: status moves emit `update` items titled "moved PROJ-1 '…' to In Review" credited to the ACTUAL actor, other edits collapse to one generic "updated …" per author (suppressed for the assignee, already credited via the `issue` item), in-window comments emit `comment` items (no bodies — privacy/tokens); caps ≤5/issue, ≤100/≤50 total; `author_email` on everything (GDPR-safe `getattr`)
- [x] Jira WIP signal — second JQL `sprint in openSprints() AND statusCategory = "In Progress" AND assignee is not EMPTY` (falls back to `updated >= -14d` variant on sites without boards), emitting `wip` items for assigned in-flight tickets untouched in the window (skipping keys the main search already returned) — quiet members with active tickets read "Continuing work on X" instead of "No activity detected."
- [x] AzDO actor + WIP — `System.ChangedBy` fetched and credited over the assignee (dict *and* "Name <email>" string identity shapes parsed); WIP WIQL (`State IN ('Active','In Progress','Doing','Committed') AND AssignedTo <> ''`) emits assignee-credited `wip` items, deduped against the changed-window ids
- [x] AzDO repo activity (new) — `azdevops_recent_commits`/`azdevops_recent_prs` via `_make_git_client()` (`GitQueryCommitsCriteria(from_date=…)`; PRs `status="all"` filtered client-side by creation/closed date — v7_1 criteria has no time filters); ≤10 repos × (50 commits + 25 PRs), 100-item total caps, one bad repo never hides the rest; wired as a separate `azdo_repos` collector source (same `azdo_project` gate) so a repo-API failure can't kill work-item collection
- [x] Confluence multi-editor — `history.createdBy/createdDate` in the CQL expand emits `page-created` items free; per-page version history (`rest/api/content/{id}/version`, ≤25 pages) emits one `page` item per DISTINCT in-window editor beyond the last modifier — three people editing one page now credit three people
- [x] GitHub feature-branch work — for ≤10 newest open/merged in-window PRs, `pr.get_commits()` emits branch commits as `commit` items ("msg (PR #N)"); collector-level dedupe (`(source, kind, key, title, author)`, sha-keyed commits drop the title so the PR-annotated duplicate of a default-branch commit collapses; local_git's constant key keeps its title) prevents double counting
- [x] Alias enrichment for ALL members — items carry `author_email` (git `%ae` added to local_git too); `engine._enrich_aliases_from_items` indexes name-alias → emails and unions each member's aliases with their observed emails (+ local parts, 2-pass closure), so a git commit as `omar.din@corp.com` attaches to Jira member "Omar Din" instead of spawning a phantom member
- [x] WIP flows distinctly to the LLM — member payload split into `activity` / `in_progress`; prompt documents the kind glossary ("commented on", "moved to …") and mandates "Continuing work on X" (never "No activity detected") for WIP-only members; deterministic fallbacks match (`_fallback_summary`); confidence's silence penalty excludes `wip` via `bundle.total(exclude_kinds=)` (standing state ≠ fresh signal)
- [x] Skipped sources visible — `ActivityBundle.skipped` (+"SDK not installed" on ImportError) → `StandupReport.skipped_sources` (defaulted, store `.get()`) → "Not scanned" rows in the Activity detail, "Sources skipped — …" line in plaintext/Markdown/HTML — unconfigured coverage is now shown, never silent
- [x] Tests — Jira changelog/comments/WIP-fallback/GDPR-email, AzDO ChangedBy/string-identity/WIP/repo helpers (caps, client-side PR filter, auth → StandupSourceError), Confluence version fan-out/page-created/failure-skip, GitHub PR commits + closed-unmerged skip, local git email, collector skip-reasons/dedupe/`total(exclude_kinds)`/azdo_repos gating, engine enrichment/phantom-member/WIP-fallback/confidence-count/skipped-sources, store + export round-trips, prompt rules; `make lint` clean, 3,909 unit+integration green (contract failures pre-existing/environmental)

## Phase 40: Standup — decluttered overview (My Update row + expandable Team row)
- [x] Overview restructured — the flat per-member section rows are gone; the section list is now Summary · Sprint · **My Update** (the standup user's own card as its own row) · **Team** (one row for everyone else) · Activity · Schedule · Notices
- [x] Inline Team expansion — Enter on the Team row toggles `team_expanded` (chevron `▸`/`▾` on the row, teaser shows "N member updates · Enter to expand/collapse"); expanded members render as indented `├`/`└` tree-guide sub-rows, each opening their existing member detail view with Enter; expansion state survives data refreshes and detail round-trips
- [x] "My Update" button removed — it fully overlapped Generate (the Generate flow already prompts for and saves the user's update first); overview buttons are now Generate · Configure · Back; dead `_standup_my_update` helper and its `_BTN_COLORS` entry deleted
- [x] `my_update` card — teaser shows the user's summary gist (+ "✍ update" when self-reported) or "No update yet — Generate asks for it"; detail reuses `_detail_member` on the resolved `my_name`; the "(you)" name tag is gone (the user is never in the Team sub-list)
- [x] Tests — card order collapsed/expanded (my card excluded from sub-rows, notices still last), my_update/team teasers, tree-guide + chevron render, my_update detail, three-button row, auto-scroll over expanded sub-rows; `make lint` clean, unit + integration green (contract failures pre-existing/environmental)
- [x] Unscanned integrations advised in the report itself — partial coverage (e.g. Jira ran but GitHub/AzDO not set up) appends one combined "Not scanned: Github (STANDUP_GITHUB_REPO not set), … — connect these in .env" line to `report.warnings`, so the ⚠ Notices card (TUI overview + detail), terminal/Slack/email delivery, and Markdown/HTML exports all carry the advice; appended last (auth/LLM problems stay on top) and suppressed when nothing is configured (the existing "No activity sources configured" notice already advises); engine tests for partial/none/all-configured cases
- [x] Links on every reference — activity items now carry a best-effort `url` from all sources (Jira `/browse/KEY` on issue/update/comment/wip, AzDO `_workitems/edit` + repo `web_url` commit/PR links, Confluence webui link with viewpage fallback, GitHub/Notion `html_url`/`url`; local git has none); the engine distils them into `MemberUpdate.links` (label = ticket key/PR/sha, deduped by URL, cap 6; URLs stripped from the LLM payload) shown as a clickable **Links** section in the member detail (OSC-8 hyperlinks + truncated URL), `↗` rows in the terminal render, raw URLs in Slack/email plaintext (clients auto-link), and `[label](url)` / `<a>` in the Markdown/HTML exports; round-trip + render + per-source URL tests
- [x] No more phantom team members — Jira/Confluence app accounts (accountType "app", e.g. "Automation for Jira", plus a display-name fallback for Server/DC) resolve to no author at the source, so bots never enter the activity feed, standup team, or roster; the tracker roster fallback now counts only assignee-credited Jira items (`include_changelog=False, include_comments=False`), so someone who merely commented on or edited a ticket in the 30-day lookback no longer appears as a team member; bot-filter + assignee-only-roster tests

## Phase 41: Standup — compact dashboard (status strip + meters + member glyphs)
- [x] Pinned status strip — one header row `Sprint {name}   Day d/t ▰▰▰▰▰▰▰▰▱▱   Behind ▰▰▱▱▱▱▱▱ 25%` built in `_build_standup_screen` (visible on every view, never scrolls away; "No standup yet — Generate creates today's standup" when no report); new shared `build_meter()` primitive in `_components.py` (confidence meter coloured good/warn/bad); one-row ⚠ banner shows the transient message or the first warning; strip/banner/subtitle are `no_wrap` and `header_h` is computed to match the Group rows exactly (drops the strip on terminals too short for it)
- [x] Deduplicated overview — the "Latest Standup" heading block, Sprint/Day/Confidence rows, "Sections" heading, hint row, and the Sprint & Confidence card are all gone (facts live in the strip once; `_detail_sprint` deleted, Day X of Y folded into the Team Summary detail); the key hint moved into the subtitle line; first section row now ~3 rows into the viewport (was ~10)
- [x] Two-row summary teaser — the Team Summary card wraps its teaser onto an aligned continuation row (ellipsized past two); `_StandupCtx.first_card_row` → `card_rows` so auto-scroll handles variable-height cards
- [x] Member glyphs + gist — expanded Team sub-rows show `├ ● name` (● active / ○ quiet+dimmed / ✍ self-reported) with the first ticket ref leading the teaser (`PSOT-9 · moved…`); Team row teaser shows `{n} updates · {a} active ● {q} quiet ○`; driven by new `MemberUpdate.activity_count` (set in both engine assembly paths, round-tripped in the store, summary-text fallback for reports saved before the field existed)
- [x] Row-wrap hardening — all overview rows `no_wrap` (a wrap silently corrupts the row-count scroll math); fixed a pre-existing off-by-one in `_StandupCtx.wrapped()` that ignored the scrollbar column and pushed the buttons off the panel on some widths
- [x] Tests — meter primitive (`TestBuildMeter`), strip/banner render, no-report strip, 2-row summary, member/team glyph teasers, expanded-row glyphs, banner-with-warnings height budget, activity_count engine + store round-trip + asdict; unit (3,408) + integration (466) green, lint/format clean
