...
The whole point of this module is that a standup fires *even when scrum-agent is
...
``scrum-agent --standup-run --standup-session <id>`` at the configured time.
...
_LABEL_PREFIX = "com.scrum-agent.standup"
_CRON_MARKER = "# scrum-agent-standup"


def _executable_args(session_id: str) -> list[str]:
    """Return the argv the scheduler should invoke — the *interactive* standup run.

    Prefer an installed ``scrum-agent`` on PATH; fall back to ``python -m
    scrum_agent.cli`` so a dev/venv install still schedules correctly. The run is
    interactive: attached to a terminal it prompts for the user's update + confirm;
    with no TTY it falls back to headless generate+deliver.
    """
    exe = shutil.which("scrum-agent")
    if exe:
        base = [exe]
    else:
        base = [sys.executable, "-m", "scrum_agent.cli"]
...
def _launcher_dir() -> Path:
    return Path.home() / "Library" / "Application Support" / "scrum-agent"
...
def _write_launcher_script(session_id: str) -> Path:
    ...
    path.write_text(f"#!/bin/sh\n# Auto-generated by scrum-agent — Daily Standup schedule.\n{cmd}\n")
    path.chmod(0o755)
    return path
